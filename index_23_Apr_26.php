...
#avgOrder, #commissionRate, #ordersPerDay {
    color: #000;
}
.z_color {
    background: red;
...
<section class="restaurant-section">
    <div class="container mx-auto px-4">
        ...
        <!-- ========== NEW COMMISSION CALCULATOR SECTION ========== -->
        <div class="bg-white rounded-2xl shadow-xl mb-12 p-6 md:p-8">
            <div class="text-center mb-8">
                <h3 class="text-3xl md:text-4xl font-extrabold text-gray-800">💰 Commission Loss Calculator</h3>
                <p class="text-gray-600 mt-2">See how much you're losing every month to delivery apps – and how DeeGeeCard saves you!</p>
            </div>

            <div class="grid md:grid-cols-3 gap-6 mb-8">
                <!-- Avg Order Value -->
                <div class="bg-gray-50 rounded-xl p-4">
                    <label class="block text-gray-700 font-semibold mb-2">🍽️ Avg. Order Value (₹)</label>
                    <input type="number" id="avgOrder" value="300" step="10" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary">
                    <input type="range" id="avgOrderSlider" min="50" max="2000" step="10" value="300" class="w-full mt-2 accent-primary">
                </div>

                <!-- Commission Rate -->
                <div class="bg-gray-50 rounded-xl p-4">
                    <label class="block text-gray-700 font-semibold mb-2">📉 <span class="z_color">Z</span> & <span class="s_color">S</span> Commission (%)</label>
                    <input type="number" id="commissionRate" value="30" step="1" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary">
                    <input type="range" id="commissionSlider" min="0" max="50" step="1" value="30" class="w-full mt-2 accent-primary">
                </div>

                <!-- Orders Per Day -->
                <div class="bg-gray-50 rounded-xl p-4">
                    <label class="block text-gray-700 font-semibold mb-2">📅 Orders Per Day</label>
                    <input type="number" id="ordersPerDay" value="10" step="1" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-primary focus:border-primary">
                    <input type="range" id="ordersSlider" min="0" max="200" step="1" value="10" class="w-full mt-2 accent-primary">
                </div>
            </div>
            ...
            <div class="grid md:grid-cols-2 gap-6">
                <div class="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-5 text-center">
                    <p class="text-gray-600 text-sm uppercase tracking-wide">Monthly Gross Business</p>
                    <p class="text-3xl md:text-4xl font-black text-blue-700" id="monthlyGross">₹0</p>
                ...
                <div class="bg-gradient-to-br from-red-50 to-red-100 rounded-xl p-5 text-center">
                    <p class="text-gray-600 text-sm uppercase tracking-wide">Monthly Commission Loss</p>
                    <p class="text-3xl md:text-4xl font-black text-red-600" id="monthlyLoss">₹0</p>
                    <p class="text-xs text-gray-500 mt-3">Paid to <span class="z_color">Z</span> & <span class="s_color">S</span></p>
                </div>
            </div>


        </div>
        <!-- ========== END COMMISSION CALCULATOR SECTION ========== -->
...
<script>
    ...
    function initCommissionCalculator() {
        ...
        const avgOrderSlider = document.getElementById('avgOrderSlider');
        ...
        const ordersInput = document.getElementById('ordersPerDay');
        const ordersSlider = document.getElementById('ordersSlider');
        
        const monthlyGrossSpan = document.getElementById('monthlyGross');
        const monthlyLossSpan = document.getElementById('monthlyLoss');
        const yearlyLossSpan = document.getElementById('yearlyLoss');
        const savingsSpan = document.getElementById('savingsVsCard');
        
        function updateCalculator() {
            let avgOrder = parseFloat(avgOrderInput.value) || 0;
            let commission = parseFloat(commissionInput.value) || 0;
            let ordersPerDay = parseFloat(ordersInput.value) || 0;
            
            avgOrderSlider.value = avgOrder;
            commissionSlider.value = commission;
            ordersSlider.value = ordersPerDay;
            
            const monthlyOrders = ordersPerDay * 30;
            const monthlyGross = avgOrder * monthlyOrders;
            const monthlyCommissionLoss = monthlyGross * (commission / 100);
            const yearlyLoss = monthlyCommissionLoss * 12;
            const savings = yearlyLoss - 14999;
            
            monthlyGrossSpan.innerText = '₹' + monthlyGross.toLocaleString('en-IN', {maximumFractionDigits:0});
            monthlyLossSpan.innerText = '₹' + monthlyCommissionLoss.toLocaleString('en-IN', {maximumFractionDigits:0});
            yearlyLossSpan.innerText = '₹' + yearlyLoss.toLocaleString('en-IN', {maximumFractionDigits:0});
            
            if (savings > 0) {
                savingsSpan.innerHTML = '₹' + savings.toLocaleString('en-IN', {maximumFractionDigits:0}) + ' 💥';
            } else if (savings < 0) {
                savingsSpan.innerHTML = '₹' + Math.abs(savings).toLocaleString('en-IN', {maximumFractionDigits:0}) + ' (but you lose commissions)';
            } else {
                savingsSpan.innerHTML = '₹0 (break-even)';
            }
        }
        
        avgOrderInput.addEventListener('input', updateCalculator);
        avgOrderSlider.addEventListener('input', function() { avgOrderInput.value = this.value; updateCalculator(); });
        commissionInput.addEventListener('input', updateCalculator);
        commissionSlider.addEventListener('input', function() { commissionInput.value = this.value; updateCalculator(); });
        ordersInput.addEventListener('input', updateCalculator);
        ordersSlider.addEventListener('input', function() { ordersInput.value = this.value; updateCalculator(); });
