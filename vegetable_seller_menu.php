...
async function checkExistingPendingOrders() {
    ...
    try {
        const response = await fetch(`check_existing_pending_vegetable_orders.php?t=${Date.now()}`);
...
function checkForNewOrders() {
    ...
    fetch(`check_new_vegetable_orders.php?last_order_id=${POLLING_CONFIG.lastOrderId}&page_load_time=${pageLoadTime}&t=${Date.now()}`)
        .then(response => response.json())
        .then(data => {
            if (data.error) {
...
function refreshPendingOrders() {
    ...
    fetch(`check_new_vegetable_orders.php?last_order_id=0&page_load_time=${POLLING_CONFIG.pageLoadTime}&t=${Date.now()}`)
        .then(response => response.json())
        .then(data => {
...
async function acceptSingleOrder(orderId) {
    ...
    try {
        ...
        const response = await fetch('accept_vegetable_orders.php', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
        ...
        if (result.success) {
            ...
            if (result.error === 'redirect_to_orders') {
                ...
                setTimeout(() => {
                    window.location.href = result.redirect_url || 'vegetable_orders.php';
                }, 1500);
            } else {
...
async function rejectSingleOrder(orderId) {
    ...
    try {
        const response = await fetch('reject_vegetable_orders.php', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
                order_ids: [orderId], 
...
function sendOrderConfirmation(orderId, customerPhone, customerName, orderType, businessInfo, businessPhone, profileUrl) {
    try {
        ...
        // Redirect to vegetable_orders.php after successful sending
        setTimeout(() => {
            window.location.href = 'vegetable_orders.php';
...
<div class="main-nav">
   ...
   <div class="scrollbar" data-simplebar>
      <ul class="navbar-nav" id="navbar-nav">
         ...
         <li class="nav-item">
            <a class="nav-link" href="vegetable_seller_dashboard.php">
               <span class="nav-icon">
         ...
         <li class="nav-item">
            <a class="nav-link" href="vegetable_orders.php">
               <span class="nav-icon">
         ...
         <li class="nav-item">
            ...
            <div class="collapse" id="services">
               <ul class="nav sub-navbar-nav">
                  ...
                  <li class="sub-nav-item">
                     <a class="sub-nav-link" href="vegetable_products.php">Vegetable Products</a>
                  </li>
                  <!-- <li class="sub-nav-item">
                  ...
                  <!-- <li class="sub-nav-item">
                     <a class="sub-nav-link" href="vegetable_time_slots.php">Delivery Time Slots</a>
                  </li> -->
                  <!-- <li class="sub-nav-item">
