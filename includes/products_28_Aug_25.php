...
function placeOrderOnWhatsApp() {
    ...
    // Calculate discount (same logic as updateCartUI)
    let discountAmount = 0;
    let discountType = '';
    let couponCode = '';
    
    // 1. Check for coupon discount first (highest priority)
    if (cart.coupon) {
        couponCode = cart.coupon.code || 'COUPON';
        if (cart.coupon.type === 'percentage') {
            discountAmount = Number((subtotal * Number(cart.coupon.value)) / 100);
    ...
    if (isDelivery && deliveryCharge !== undefined) {
        if (freeDeliveryMin > 0 && amountAfterDiscount >= freeDeliveryMin) {
            ...
            deliveryText = `Delivery:       FREE\n` +
                           `(Order above ₹${formatNumber(freeDeliveryMin)})`;
        } else {
            // Apply normal delivery charge
            actualDeliveryCharge = parseFloat(deliveryCharge);
            deliveryText = `Delivery:       ₹${formatNumber(deliveryCharge)}`;
            if (freeDeliveryMin > 0) {
                const neededForFree = freeDeliveryMin - amountAfterDiscount;
                deliveryText += `\n(Add ₹${formatNumber(neededForFree)} more for FREE delivery)`;
            }
        }
    }
    ...
    if (discountAmount > 0) {
        ...
        // Show different discount types with appropriate formatting
        if (cart.coupon) {
            // For coupon discounts, show the full discount type with coupon code
            message += `(${discountType})\n`;
        } else {
            // For automatic discounts, show the discount type
            message += `(${discountType})\n`;
        }
    }
