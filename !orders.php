...
<!-- Add this to your orders.php file before closing </body> tag -->
<script>
class RealTimeOrderUpdates {
    constructor() {
        this.ws = null;
    ...
    connect() {
        try {
            ...
            this.ws.onopen = () => {
                console.log('Secure WebSocket connected for real-time order updates');
                this.reconnectAttempts = 0;
    ...
    handleUpdate(data) {
        if (data.type === 'order_update') {
            ...
            if (data.user_id && data.user_id.toString() === '<?php echo $_SESSION["user_id"] ?? 0; ?>') {
                this.refreshOrdersTable();
                this.showNotification(`Order #${data.order_id} was ${data.new_status} from another device`);
            }
        }
    }

    refreshOrdersTable() {
        // Use your existing refresh logic
        if (typeof window.refreshOrders === 'function') {
            ...
            // Reload only if we're on orders page
...
document.addEventListener('DOMContentLoaded', function() {
    // Only initialize on orders page and if user is logged in
    if (window.location.pathname.includes('orders.php') && <?php echo isset($_SESSION['user_id']) ? 'true' : 'false'; ?>) {
        window.realTimeUpdates = new RealTimeOrderUpdates();
        console.log('Real-time order updates initialized for deegeecard.com');
    }
});
</script>
