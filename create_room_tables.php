...
$tables = [
    // Rooms table - Enhanced with more fields
    "rooms_$user_id" => "
    ...
    // Room rates table (seasonal pricing) - Enhanced
    "room_rates_$user_id" => "
    ...
    // Guests table - Enhanced with loyalty
    "guests_$user_id" => "
    ...
    // Room maintenance table - Enhanced
    ...
    // In the bookings table definition, replace it with this enhanced version:
    "bookings_$user_id" => "
        CREATE TABLE IF NOT EXISTS `bookings_$user_id` (
            `id` INT(11) NOT NULL AUTO_INCREMENT,
            `booking_reference` VARCHAR(50) NOT NULL,
    ...
    // Housekeeping schedule table - Enhanced
    ...
    // Payments table - Enhanced with more details
    "payments_$user_id" => "
    ...
    // Loyalty program table - Enhanced
...
// Add these tables to the existing $tables array in create_room_tables.php
...
// Add this to your existing $tables array in create_room_tables.php
...
<body>
                                ...
                                <div class="card bg-light">
                                    <div class="card-body text-center">
                                        ...
                                        <div class="d-flex flex-wrap justify-content-center">
                                            ...
                                            <a href="room-dashboard.php" class="btn btn-primary action-btn">
                                        ...
                                        <small class="text-muted mt-2 d-block">
                                            Force recreate will drop all existing tables and create fresh ones
                                        </small>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-sm table-bordered table-hover">
                                ...
                                <tbody>
                                    <?php 
                                    $table_descriptions = [
                                        "rooms_$user_id" => ["Room inventory and status management", "Rooms"],
                                        "room_types_$user_id" => ["Room categories and base configurations", "Room Types"],
                                        "room_rates_$user_id" => ["Seasonal pricing and special rates", "Rate Plans"],
                                        "bookings_$user_id" => ["Reservations and booking management", "Bookings"],
                                        "guests_$user_id" => ["Guest information and profiles", "Guests"],
                                        "room_amenities_$user_id" => ["Room facilities and features", "Amenities"],
                                        "room_maintenance_$user_id" => ["Maintenance scheduling and tracking", "Maintenance"],
                                        "housekeeping_$user_id" => ["Cleaning schedules and task management", "Housekeeping"],
                                        "payments_$user_id" => ["Payment transactions and records", "Payments"],
                                        "loyalty_program_$user_id" => ["Guest loyalty and rewards program", "Loyalty"],
                                        "tax_settings_$user_id" => ["Tax configurations and settings", "Taxes"],
                                        "discounts_$user_id" => ["Discount offers and promotions", "Discounts"],
                                        "staff_members_$user_id" => ["Staff management and assignments", "Staff"],
                                        "extra_charges_$user_id" => ["Additional charges and services", "Extra Charges"],
                                        "audit_logs_$user_id" => ["System audit and activity logs", "Audit Logs"]
                                    ];
                                    
                                    foreach ($tables as $table_name => $query): 
                                        $description = $table_descriptions[$table_name] ?? ['General table', 'Records'];
                                    ?>
                                        <tr>
                                            <td>
                                                <code><?php echo $table_name; ?></code>
                                                <?php if (strpos($table_name, '_new') !== false): ?>
                                                    <span class="badge bg-info ms-1">NEW</span>
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                <strong><?php echo $description[0]; ?></strong>
                                                <br><small class="text-muted">Manages <?php echo strtolower($description[1]); ?></small>
                                            </td>
                                            <td>
                                                <span class="badge bg-secondary">0</span> records
                                            </td>
                                            <td>
                            ...
                            <div class="col-md-4">
                                <div class="card h-100">
                                    <div class="card-body text-center">
                                        ...
                                        <h6>1. Basic Setup</h6>
                                        <p class="small text-muted">Configure room types, amenities, and tax settings</p>
    ...
    <script>
        // Auto-redirect to dashboard after 8 seconds if successful and no errors
        <?php if (empty($error_messages) && !empty($success_messages)): ?>
            setTimeout(function() {
                window.location.href = 'room-dashboard.php';
            }, 8000);
        <?php endif; ?>

        // Add some interactive features
        document.addEventListener('DOMContentLoaded', function() {
            // Add click handlers for table rows to show more details
